# app/etl_central/pipelines/egresos_detallado_cp_single_pipeline.py
import os
...
def _latest_year(bucket: str, logger) -> int | None:
    years = find_all_egresos_detallado_cp_years(bucket_name=bucket, prefix=CP_S3_PREFIX)
...
def pipeline(pipeline_logging: PipelineLogging):
    ...
    logger.info("100 | Starting Egresos Detallado CP single pipeline run")
...
def run_egresos_cp_single_pipeline(pipeline_name: str, log_client: PostgreSqlClient):
    ...
    try:
        ...
        pipeline_logging.logger.error(f"500 | Egresos CP single pipeline run failed: {e}")
        metadata_logger.log(
...
if __name__ == "__main__":
    ...
    PIPELINE_NAME = "egresos_detallado_cp_single_pipeline"
    run_egresos_cp_single_pipeline(pipeline_name=PIPELINE_NAME, log_client=log_client)
