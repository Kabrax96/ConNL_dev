# app/etl_central/pipelines/ingresos_detallado_cp_bulk_pipeline.py
import os
import pandas as pd
...
def pipeline(pipeline_logging: PipelineLogging):
    ...
    logger.info("100 | Starting Ingresos Detallado CP bulk pipeline run")
    ...
    years = find_all_ingresos_detallado_cp_years(bucket_name=BUCKET_NAME, prefix=CP_S3_PREFIX)
    ...
    logger.info("499 | Ingresos CP bulk pipeline run successful")

def run_ingresos_cp_bulk_pipeline(pipeline_name: str, log_client: PostgreSqlClient):
    ...
    try:
        ...
        pipeline_logging.logger.error(f"500 | Ingresos CP bulk pipeline run failed: {e}")
        metadata_logger.log(
...
if __name__ == "__main__":
    ...
    PIPELINE_NAME = "ingresos_detallado_cp_bulk_pipeline"
    run_ingresos_cp_bulk_pipeline(pipeline_name=PIPELINE_NAME, log_client=log_client)
